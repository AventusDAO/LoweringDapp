...
const ERC1820_REGISTRY_CREATE2_ADDRESS = '0x1820a4B7618BdE71Dce8cdc73aAB6C95905faD24'; // Same address for mainnet and rinkeby
const ERC777_INTERFACE_HASH = 'ac7fbab5f54a3ca8194167523c6753bfeb96a445279294b6125b68cce2177054';
...
async function fullDecimalAmount(amount, token, isERC777) {
    ...
    if (token) {
      ...
      decimals = await load.eth.call({ to: token, data: DECIMALS_FUNC_SELECTOR }).catch(error => false);
      if (!decimals) return null;

      if (isERC777 && await implementsERC777(load, token) === false) return null;
    }
...
async function implementsERC777(load, token) {
  const paddedToken = '000000000000000000000000' + token.toLowerCase().substring(2);
  const data = ERC1820_INTERFACE_IMPLEMENTER_FUNC_SELECTOR + paddedToken + ERC777_INTERFACE_HASH;
  return await load.eth.call({ to: ERC1820_REGISTRY_CREATE2_ADDRESS, data }) === '0x' + paddedToken;
}

export default fullDecimalAmount;
